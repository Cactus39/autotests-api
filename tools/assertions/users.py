...
def assert_get_user_response(get_user_response: GetUserResponseSchema, create_user_response: CreateUserResponseSchema):
    ...
    :param get_user_response: Ответ API с данными пользователя после вызова метода get_user_api.
    :param create_user_response: Ответ API с данными пользователя после создания пользователя.
    :raises AssertionError: Если хотя бы одно поле не совпадает.
    """
